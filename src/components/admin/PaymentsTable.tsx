    ...
    () => [
      {
        ...
        header: "Date",
        cell: ({ row }) => {
          const date = format(new Date(row.original.created_at), "MMM dd, yyyy");
          const time = format(new Date(row.original.created_at), "HH:mm");
          return (
            <div>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="text-sm font-medium">{service.name}</span>;
        },
      },
      {
        id: "amount",
      ...
      },
      {
        ...
        header: "Stripe",
        cell: ({ row }) => {
          const paymentIntentId = row.original.stripe_payment_intent_id;
          if (!paymentIntentId) return <span className="text-xs text-gray-400">N/A</span>;
          return (
            <a
